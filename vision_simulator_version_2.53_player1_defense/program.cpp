////
//FINAL VERSION <-- program (cpp) [Defense - Player 1]
...
// Obstacle 2 Position
//double init_x_obs2 = 450;
//double init_y_obs2 = 200;
...
//	size_obs[2] = 1.0; // scale factor 1.0 = 100% (not implemented yet)	
	...
	mask_width = 300;
	mask_height = 200;
	stopping_distance_obstacle = 180.00;
	...
	while (1)
		{
				...
				//		input:	obstacle (#, size, x, y)		robot(x, y, th)		enemy(x, y, th)
				//		output:	xgoal, ygoal (ideal -> safe location)	phi_goal (ideal -> safe orientation)
				//		description:	
				//					1. determines types of obstacles present 
				//							centered(bounded) --> circling, 
				//							bordering(unbounded) --> park and face outwards (keeping head away from center of playing field)
				//					2. enter the different scenarios 5 in total 
				//							scenario 1 - 1 bounded obstacle		scenario 3 - 1 unbounded obstacle
				//							scenario 2 - 2 bounded obstacle		scenario 4 - 2 unbounded obstacle		scenario 5 - 1 bounded, 1 unbounded obstacle
				//					3. mostly depending on location of obstacle (and sometimes robot, enemy position,orientation)
				//							produce ideal safe location (xgoal, ygoal)
				//							produce ideal safe orientation (phi_goal)
				defend(obs_number, x_obs_array, y_obs_array, obstacles_size, xgoal, ygoal, phi_goal, x_robot, y_robot, phi_robot, x_opp, y_opp, phi_opp);

				double phi_goal_move = atan2(ygoal - y_robot, xgoal - x_robot)*(180.00 / 3.14159);
				...
				tf_workspace_to_mask(xgoal, ygoal, mask_ci, mask_cj, mi_target, mj_target, mask_angle);
				...
				//For goal point from defend function inside the mask
				if ((mi_target > 0 && mi_target < mask_width) && (mj_target>0 && mj_target < mask_height))
				{

					//Safe point reached stop and align

					if (goal_distance < 5.00) //size_target + buffer to avoid bumping into the opponent
					{
				...
				// For Obstacle inside the mask 
				 else if((mi_inside > 0 && mi_inside < mask_width) && (mj_inside >0 && mj_inside < mask_height))
						{
							if (obs_distance[num_near] < 200.00)
							{
							block =1;
							}
						}
				...
				// To control the movement of the function irrespective of the fact that something is within mask or not
						...
						{
							// Move toawrds safe point

							double error_tolerance = 5.0;
							double e_orientation_move;

							e_orientation_move = phi_goal_move - phi_robot;

							//Align if out of Tolerance 
							if (e_orientation_move >= error_tolerance || e_orientation_move <= -error_tolerance)
							{
								align_full_throttle(phi_robot, phi_goal_move, pw_l, pw_r);
							}
				 ...
				 if (block && !safe_sight)
				 { 
					 // We have entertered bounded region
					 bounded = 1;
				...
				if (obs_distance[num_near] < 100.00)
				{
					move_backward(pw_l, pw_r, ep, ei);
				}
